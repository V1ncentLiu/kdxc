...
YX.fn.myInfoEvt = function () {
    ...
    $('#statusBoxClick li').on('click', this.statusBoxClick.bind(this))
    ...
    // this.$userPic.delegate('#userPic', 'click', this.showModifyAvatar.bind(this,"my"))
    $('#userPic').on('click', this.showModifyAvatar.bind(this, "my"))
    //修改头像
...
YX.fn.statusBoxClick = function (e) {
    ...
    if (tar[0].localName == 'li') {
        ...
        var i = tar.parent().parent().children().index(tar.parent())
    }
    this.changeState(i)
    $(".statusBox").hide();
}
...
YX.fn.changeState = function (type) {
    var params = {
        teleSaleId: '',
        operationType: '',
        type: 1
