...
var Cache = (function() {
  ...
  Cache.prototype.sessionsListConcat = function (sessions) {
    ...
    // $.ajax({
    //     url: CONFIG.url + '/im/brandAndIssubmit',
    //     type: 'POST',
    //     data: params,
    ...
    for(let i=0;i<sessions.length;i++){
        for(let j=0;j<array.length;j++){
            if(sessions[i].to==array[j].imId&&sessions[i].scene=='p2p'){
                newArray.push(Object.assign(sessions[i], array[j]))
            }
        }
    }
    return newArray&&newArray.slice(0,array.length)
}
