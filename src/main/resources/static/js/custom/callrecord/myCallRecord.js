var myCallRecordVm = new Vue({
    ...
    methods:{
      transCusPhone(row) {
        var text="";
        if( (roleCode =='DXCYGW' && row.accountId!=userId) || (row.clueId != null &&( row.phase ==7 || row.phase == 8 || (roleCode =='DXCYGW' && (row.teleSaleId+"") != userId) ||  (roleCode =='DXZJ' && orgId !=(row.teleGorupId+""))))){
          text ="***"
        }else{
