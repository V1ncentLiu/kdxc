var clientVm = new Vue({
     ...
     methods: {
         ...
         addClientDialog(){//添加坐席弹窗
        	 ...
        	 this.submitClientUrl ='saveZkClient';
         ...
         deleteClientDialog(){//删除坐席弹窗
             ...
             for(var i=0;i<rows.length;i++){
                 ...
                 rowNames.push("【"+curRow.loginClient+"】");
             }
             var clientNos = rowNames.join(" ");


             this.$confirm('确定要删除 '+clientNos+'登录坐席？', '提示', {
                 ...
                 type: 'warning'
        	      ...
        	      axios.post('/client/zkClient/deleteZkClient', param)
                  .then(function (response) {
                      var resData = response.data;
                      if(resData.code=='0'){
         ...
         modifyClientDialog(){
        	 ...
        	 this.submitClientUrl ='updateZkClient';
        	 if (this.$refs['clientForm']!==undefined) {
          		this.$refs['clientForm'].resetFields();
          	}
             ...
             .then(function (response) {
                 ...
                 axios.post('/client/zkClient/queryZkClientById', param)
                 .then(function (response) {
                     var resData = response.data;
                     if(resData.code=='0'){
                        ...
                        clientVm.confirmBtnDisabled=false;//启用提交按钮
                  	     ...
                  	     clientVm.$message({message:'查询中科坐席失败',type:'error'});
                        //  console.error(resData);
                         clientVm.confirmBtnDisabled=false;//启用提交按钮
                     }
         ...
         saveClient(formName){//保存坐席
        	 this.$refs[formName].validate((valid) => {
                 if (valid) {
                    ...
                    var param=this.form;
                   axios.post('/client/zkClient/'+this.submitClientUrl, param)
                   .then(function (response) {
                       var resData = response.data;
                       if(resData.code=='0'){
         ...
         initClientData(){
        	 ...
        	 axios.post('/client/zkClient/listZkClientPage',param)
             .then(function (response) {
           ...
           submitUploadClient(){//上传坐席
               ...
               this.$confirm('确认导入数据','提示', {
                   ...
                   type: 'warning'
              	   ...
              	   axios.post('/client/zkClient/submitZkClientData',{})
                     .then(function (response) {
                    	 var data =  response.data;
                         if(data.code=='0'){
                            var resData =  data.data;
     ...
     mounted(){
     	document.getElementById('zkClientManage').style.display = 'block';
     },
})