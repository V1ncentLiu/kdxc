var clientVm = new Vue({
     ...
     data: {
         ...
         rules:{
             ...
             okccAccount:[
                 { required: true, message: '赤晨外呼账号不能为空'},
             ],
             okccPassword:[
                 { required: true, message: '赤晨外呼密码不能为空'},
             ],
        	 userId:[
     ...
     methods: {
         ...
         modifyClientDialog(){
        	 ...
        	 this.submitClientUrl ='updateOkccClient';
        	 if (this.$refs['clientForm']!==undefined) {
          		this.$refs['clientForm'].resetFields();
          	}
                 ...
                 .then(function (response) {
                     ...
                     if(resData.code=='0'){
                        ...
                        clientVm.confirmBtnDisabled=false;//启用提交按钮
                  	     ...
                  	     clientVm.$message({message:'查询赤晨坐席失败',type:'error'});
                        //  console.error(resData);
                         clientVm.confirmBtnDisabled=false;//启用提交按钮
                     }
     ...
     mounted(){
     	document.getElementById('okccClientManage').style.display = 'block';
     },
})