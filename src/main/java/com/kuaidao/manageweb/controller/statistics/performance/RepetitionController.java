...
public class RepetitionController extends BaseStatisticsController {
    ...
    public String repetitionTable(HttpServletRequest request) {
        ...
        if (RoleCodeEnum.DXCYGW.name().equals(roleCode)) {
            initBaseDto(request, null, curLoginUser.getOrgId(), curLoginUser.getId(), null, null,
                    null, null, null, null);
    ...
    public String selfVisitFollowTableTeam(HttpServletRequest request, Long teleDeptId,
            Long teleGroupId, Long teleSaleId, Long startTime, Long endTime, String strSignStore,
        ...
        initBaseDto(request, teleDeptId, teleGroupId, teleSaleId, startTime, endTime, strSignStore,
                signType, projectId, companyId);
        return "reportPerformance/repetitionTableTeam";
    }
    ...
    public @ResponseBody JSONResult<Map<String, Object>> queryByPage(
            @RequestBody DupOrderQueryDto baseQueryDto) {
        // 查询组权限初始化
    ...
    public @ResponseBody JSONResult<Map<String, Object>> querySaleByPage(
            @RequestBody DupOrderQueryDto baseQueryDto) {
        UserInfoDTO curLoginUser = CommUtil.getCurLoginUser();
        String roleCode = curLoginUser.getRoleList().get(0).getRoleCode();
        if (RoleCodeEnum.DXCYGW.name().equals(roleCode)) {
    ...
    public @ResponseBody void export(@RequestBody DupOrderQueryDto baseQueryDto,
        ...
        try {
            ...
            if (null != json && "0".equals(json.getCode())) {
                DupOrderDto[] dtos = json.getData().isEmpty() ? new DupOrderDto[] {}
                        : json.getData().toArray(new DupOrderDto[0]);
                ...
                String name = MessageFormat.format("重单表_{0}_{1}.xlsx",
                        "" + baseQueryDto.getStartTime(), baseQueryDto.getEndTime() + "");
    ...
    public @ResponseBody void exportSale(@RequestBody DupOrderQueryDto baseQueryDto,
            HttpServletResponse response) {
        try {
            ...
            if (null != json && "0".equals(json.getCode())) {
                ...
                Workbook wb = ExcelUtil.createWorkBook(dtos, keys, hader);
                String name = MessageFormat.format("顾问重单表_{0}_{1}.xlsx",
                        "" + baseQueryDto.getStartTime(), baseQueryDto.getEndTime() + "");
                response.addHeader("Content-Disposition", "attachment;filename=\"" + name + "\"");
                response.addHeader("fileName", URLEncoder.encode(name, "utf-8"));
                response.setContentType("application/octet-stream");
    ...
    public void initBaseDto(HttpServletRequest request, Long deptId, Long groupId, Long saleId,
            Long startTime, Long endTime, String shopTypes, Integer signType, Long projectId,
            Long companyId) {
    ...
    /**
     * 参数控制权限-已经显示结果 一级列表所有权限筛选由 组id控制
     * 
     * @param baseQueryDto
     */
