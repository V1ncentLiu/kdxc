...
public class SignRecordController {
  ...
  private List<UserInfoDTO> getUserInfo(Long orgId, String roleName) {
    ...
    if (userJr == null || !JSONResult.SUCCESS.equals(userJr.getCode())) {
      logger.error("查询电销通话记录-获取组内顾问-userInfoFeignClient.listByOrgAndRole(req),param{{}},res{{}}",
          req, userJr);
      return null;
    }
  ...
  public JSONResult listPayDetailNoPage(@RequestBody IdListLongReq idListLongReq) {
    ...
    Map payDetailMap =
        data.stream().collect(Collectors.groupingBy(PayDetailDTO::getPayType, Collectors.toList()));
