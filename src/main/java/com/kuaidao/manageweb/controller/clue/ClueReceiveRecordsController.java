...
public class ClueReceiveRecordsController {
	...
	public JSONResult<ClueReceiveRecordsDTO> receiveClueByClueIds(@RequestBody ClueReceiveRecordsDTO clueReceiveRecordsDTO) {
		...
		if (roleList != null && !RoleCodeEnum.DXZJ.name().equals(roleList.get(0).getRoleCode()) && !RoleCodeEnum.DXCYGW.name().equals(roleList.get(0).getRoleCode())) {
			return new JSONResult().fail(AggregationErrorCodeEnum.PUBLICCLUERECEIVERLOE.getCode(), AggregationErrorCodeEnum.PUBLICCLUERECEIVERLOE.getMessage());
		}else {
