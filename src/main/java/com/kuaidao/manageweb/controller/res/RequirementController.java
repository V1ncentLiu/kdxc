...
public class RequirementController extends BaseStatisticsController {
    ...
    public @ResponseBody JSONResult<PageBean<ResRequirement>> quetyPage(
            @RequestBody ResQueryDto dto) {
        // initDto(dto);
        UserInfoDTO user = CommUtil.getCurLoginUser();
        dto.setUserDataAuthList(user.getUserDataAuthList());
    ...
    public @ResponseBody void export(HttpServletResponse response, @RequestBody ResQueryDto dto) {
        ...
        try {
            ...
            if ("0".equals(json.getCode())) {// && !json.getData().isEmpty()
                ResRequirement[] dtos = json.getData().isEmpty() ? new ResRequirement[] {}
                        : json.getData().toArray(new ResRequirement[0]);
                String[] keys = {"months", "teleGroupName", "projectNames", "projectType",
                        "biddingPrice", "optimization", "information", "remarks"};
                String[] hader =
                        {"月份", "电销组", "电销组项目", "项目类别", "竞价(日均量)", "优化(日均量)", "信息流(日均量)", "备注"};
                Workbook wb = ExcelUtil.createWorkBook(dtos, keys, hader);
                String name =
                        MessageFormat.format("资源需求_{0}.xlsx", "" + System.currentTimeMillis() + "");
        ...
        if (RoleCodeEnum.DXZJL.name().equals(roleCode)) {
            if (null != dto.getTeleDeptId()) {
                ...
                JSONResult<List<OrganizationRespDTO>> result =
                        organizationFeignClient.queryOrgByParam(queryDTO);
                List<Long> ids =
                        result.getData().stream().map(c -> c.getId()).collect(Collectors.toList());
                dto.setDeptIds(ids);
            }
