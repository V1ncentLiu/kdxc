...
public class MerchantApplyInvoiceController {
    ...
    public String initApplyInvoiceRecord(HttpServletRequest request) {
        ...
        JSONResult<List<UserInfoDTO>> userInfoDTOList = merchantUserInfoFeignClient.merchantUserList(userInfoDTO);
        if (CollectionUtils.isNotEmpty(userInfoDTOList.getData())) {
            request.setAttribute("userList", userInfoDTOList);
        }
     ...
     * @return
     */
    @RequestMapping("/findMerchantApplyInvoiceDeatil")
    @ResponseBody
    public JSONResult<MerchantApplyInvoiceDTO> findMerchantApplyInvoiceDeatil(@RequestBody IdEntityLong req) {
        return merchantApplyInvoiceFeignClient.findMerchantApplyInvoiceDeatil(req);
    }
