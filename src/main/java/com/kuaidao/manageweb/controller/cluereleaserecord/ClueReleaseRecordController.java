...
public class ClueReleaseRecordController {
	...
	public JSONResult<PageBean<ReleaseRecordRespDTO>> queryPageList(@RequestBody ReleaseRecordReqDTO dto) {
		...
		/**
		 * 数据权限说明： 管理员权限： 能够看见全部数据 电销顾问： 能够看见自己以及所在电销组下电销创业顾问创建的数据 电销副总：
		 * 能够看见事业部下所有人的释放记录
		 *
	...
	/**
	 * 电销副总： 管理多个电销组 一个电销组中： 一个电销总监，多个电销创业顾问 电销副总：能够看见整个事业部的释放记录 就是能够看见：当前组织 以及
	 * 当前组织的全部下属组织机构
	 *
	...
	private List allUser() {
		...
		JSONResult<List<OrganizationDTO>> orgResult = organizationFeignClient
	...
	public JSONResult<List<ReleaseRecordRespDTO>> getReleaseRecordRespByClueId(@RequestBody ReleaseRecordReqDTO dto) {
